...
# Priority order. Enrichment is what builds the context; triage is the chatty one.
# When the budget is tight the reactor yields first and the brief is the last thing to stop.
# Enrichment is what the reserve protects now. The brief it used to protect is gone, and of
...
def allowed(conn, job: str) -> tuple[bool, str]:
    ...
    s = state(conn)
    protected = PRIORITY.get(job, 3) == 0

    # (window label, spent, ceiling, how much of it the protected job keeps to itself)
